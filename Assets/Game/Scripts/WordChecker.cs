...
public class WordChecker : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            Debug.DrawRay(_rayUp.origin, _rayUp.direction * 4f);
            Debug.DrawRay(_rayDown.origin, _rayDown.direction * 4f);
            Debug.DrawRay(_rayLeft.origin, _rayLeft.direction * 4f);
            Debug.DrawRay(_rayRight.origin, _rayRight.direction * 4f);
            Debug.DrawRay(_rayDiagonalLeftUp.origin, _rayDiagonalLeftUp.direction * 4f);
            Debug.DrawRay(_rayDiagonalLeftDown.origin, _rayDiagonalLeftDown.direction * 4f);
            Debug.DrawRay(_rayDiagonalRightUp.origin, _rayDiagonalRightUp.direction * 4f);
            Debug.DrawRay(_rayDiagonalRightDown.origin, _rayDiagonalRightDown.direction * 4f);
        }
    }
    ...
    private void SquareSelected(string letter, Vector3 squarePosition, int squareIndex)
    {
        ...
        else if(_assignedPoints == 1)
        {
            ...
            GameEvents.OnSelectSquareMethod(squarePosition);
    ...
    private void CheckWord()
    {
        foreach(var searchingWord in currentGameData.selectedBoardData.SearchWords)
        {
            if(_word == searchingWord.Word)
            {
                ...
                LastOrAllWordsCount.Singleton.DecreaseLastWordsCount();
                return;
            }
        }
    }
